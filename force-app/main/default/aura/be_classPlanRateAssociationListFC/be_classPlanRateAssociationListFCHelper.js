...
({
    ...
    populateDetailGrid : function(component, helper) {
        ...
        selectedClassPlans.forEach( function(selectedClassPlan){
            selectedPlanIds.add('\'' + selectedClassPlan["Predefined_Plan__c"] + '\'');
            selectedComponentCodes.add('\'' + selectedClassPlan["Product_Component_Code__c"] + '\'');
        });
        var querymap = {};
        querymap['object'] = 'PredefinedPackageBenefit__c';
        querymap['queryFields'] = 'Id, Predefined_Package__c, Component_Type__c,Is_Base_Component_Benefit__c, Is_Key_Benefit__c,Attribute_Category__c, Attribute_Code__c, Attribute_Display_Name__c,Attribute_Value_Amount__c,In_Network_Display_Name__c,Out_Of_Network_Display_Value__c ';
        var filter = '';
        filter += 'Predefined_Package__c IN (' + [...selectedPlanIds].join(',') + ')';
        ...
        .then(
            $A.getCallback(function(result) {
                ...
                if(isSuccess == true){
                    var responsedata = responsemsg.results.data[0].PredefinedPackageBenefit__c;
                    component.set("v.predefinedPackageBenefits", responsedata);
                    var planBenefitMapList = [];
                    responsedata.forEach(function(predefinedPackageBenefit){
                        planBenefitMapList = planBenefitMap[predefinedPackageBenefit["Predefined_Package__c"]];
                        if($A.util.isUndefinedOrNull(planBenefitMapList)){
                           planBenefitMapList = [];
                        }
                        planBenefitMapList.push(predefinedPackageBenefit);
                        planBenefitMap[predefinedPackageBenefit["Predefined_Package__c"]] = planBenefitMapList;
                    });
                    component.set("v.planBenefitMap", planBenefitMap);

                    var querymap = {};
                    querymap['object'] = 'BaseRateVariant__c';
        ...
        .then(
            $A.getCallback(function(result) {
                ...
                var isSuccess = responsemsg.isSuccess;
                if(isSuccess == true){
                    var responsedata = responsemsg.results.data[0].BaseRateVariant__c;
                    var idx = 0;
                    proposedRates = [];
                    responsedata.forEach(function(baseRateVariant){
                        selectedClassPlans.forEach(function(selectedClassPlan){
                            if(selectedClassPlan.Product_Component_Code__c === baseRateVariant.Variant_Code__c){
